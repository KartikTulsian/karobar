...
export function POParties({ po, tenant }: { po: PurchaseOrderDetail, tenant: Tenant }) {
    return (
        <div className="flex flex-col sm:flex-row justify-between gap-6 py-6 border-b border-slate-200 dark:border-slate-700/50">
            ...
            <div className="flex flex-col gap-1.5 w-1/2">
                <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Vendor (From)</p>
                <h3 className="font-bold text-slate-900 dark:text-white text-base">{po.suppliers?.name || 'Unknown Supplier'}</h3>
                ...
                <div className="mt-1">
                    {po.suppliers?.phone && <p className="text-sm text-slate-600 dark:text-slate-400"><span className="font-medium">Ph:</span> {po.suppliers.phone}</p>}
                    {po.suppliers?.email && <p className="text-sm text-slate-600 dark:text-slate-400"><span className="font-medium">Email:</span> {po.suppliers.email}</p>}
                    {po.is_gst_supply && po.suppliers?.gstin && (
            ...
            {/* Shop Details (Dynamic Tenant Data) */}
            <div className="flex flex-col gap-1.5 sm:text-right w-1/2">
                <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Ship To / Bill To</p>
